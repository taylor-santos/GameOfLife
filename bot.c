...
int *minimax(const struct State *state, const struct State **predictions, const unsigned char id, const unsigned char depth, int alpha, int beta)
{
		...
		for (int x=0; x<FIELD_WIDTH; x++){
			...
			if (state->neighbors[index] != 0) {
				...
				if (owner == 0) {
					...
					/*else if (owner == 1 && n0 == 2 && n1 == 1) {
						priority2_cells[priority2_index++] = index;
					}
					else if (owner == 2 && n1 == 2 && n0 == 1) {
						priority2_cells[priority2_index++] = index;
					}*/
					else {
	...
	for (int i = 0; i < pb+npb; i++) {
		...
		set_cell(state1, birth_index, id+1);
	...
	*/
	...
	for (int i = 0; i < 288; i++) {
		if (cutoff_k[i]){
			...
			int index = i;
			if (id == 0) {
				count[0][state->neighbors[index] % 9][state->neighbors[index] / 9] += mul;
			}
			else {
				count[0][state->neighbors[index] / 9][state->neighbors[index] % 9] += mul;
			}
		...
		if (cutoff_b[i]){
			...
			int index = i;
			if (id == 0) {
				count[1][state->neighbors[index] % 9][state->neighbors[index] / 9] += mul;
			}
			else {
				count[1][state->neighbors[index] / 9][state->neighbors[index] % 9] += mul;
			}
